...
export const getStats = async (req, res) => {
  try {
    ...
    // 2. Sales Data (Graph) - Last 6 Months & Last 7 Days & Yearly & All Time
    let dailySales = [], weeklySales = [], monthlySales = [], yearlySales = [], allTimeSales = [];
    ...
    try {
      ...
      const salesResults = await Promise.allSettled([
        ...
        getSalesData('year', 3650),
        getSalesData('allTime', null)
      ]);
      ...
      yearlySales = salesResults[3].status === 'fulfilled' ? salesResults[3].value : [];
      allTimeSales = salesResults[4].status === 'fulfilled' ? salesResults[4].value : [];
