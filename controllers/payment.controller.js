...
import { generateSlipBuffer } from "../utils/generateSlip.js";
...
export const withdrawWallet = async (req, res) => {
  try {
    ...
    // Enforce 500 minimum for ambassadors
    if (user.isAmbassador && user.walletBalance < 500) {
      return res.status(400).json({ success: false, message: "Ambassadors can only withdraw when balance is at least ₹500" });
    }
    ...
    if (user.mobile) {
      const smsMessage = `Your withdrawal of Rs.${amount} was successful. Remaining balance: Rs.${user.walletBalance}.`;
...
export const downloadTransactionSlip = async (req, res) => {
  try {
    const { id } = req.params;
    ...
    const pdfBuffer = await generateSlipBuffer(payment, user);

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', `attachment; filename=slip_${payment._id}.pdf`);
    res.send(pdfBuffer);
  } catch (err) {
