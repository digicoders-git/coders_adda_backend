...
export const getSalesDashboardData = async (req, res) => {
  try {
    ...
    if (chartFilter === "yearly") daysBack = 3650;
...
export const getItemSalesData = async (req, res) => {
  try {
    ...
    if (chartFilter === "yearly") daysBack = 365;

    const periodMatchStage = { itemType, status: "success" };
    if (chartFilter !== "allTime") {
      const startDate = new Date();
      startDate.setDate(startDate.getDate() - daysBack);
      startDate.setHours(0, 0, 0, 0);
      periodMatchStage.createdAt = { $gte: startDate };
    }
    ...
    if (chartFilter === "yearly") chartDaysBack = 3650;

    const chartMatchStage = { itemType, status: "success" };
    if (chartFilter !== "allTime") {
      const chartStartDate = new Date();
      chartStartDate.setDate(chartStartDate.getDate() - chartDaysBack);
      chartMatchStage.createdAt = { $gte: chartStartDate };
    }
