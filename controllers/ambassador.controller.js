...
export const adminApproveApplication = async (req, res) => {
  try {
    ...
    const notification = new Notification({
      ...
      body: `Congratulations ${application.fullName}! You are now a Campus Ambassador. Your referral code is ${generatedCode}.`,
      priority: "High",
...
export const adminRejectApplication = async (req, res) => {
  try {
    ...
    const notification = new Notification({
      ...
      body: `Hi, your application for Campus Ambassador was not approved at this time. Reason: ${application.adminComment}`,
      priority: "Normal",
      targetGroup: "Specific",
      targetUsers: [application.user.toString()],
      status: "Pending"
    });
    await notification.save();
...
// Block/Unblock Ambassador
export const adminBlockUnblockAmbassador = async (req, res) => {
  try {
    const { id } = req.params;
    const { action, comment } = req.body; // action: 'block' or 'unblock'
    const application = await AmbassadorApplication.findById(id);
    ...
    application.status = action === 'block' ? 'blocked' : 'approved';
    application.adminComment = comment || (action === 'block' ? "Blocked by admin" : "Unblocked by admin");
    await application.save();

    const user = await User.findById(application.user);
    if (user) {
      user.isAmbassador = action === 'unblock';
      await user.save();

      const smsMessage = action === 'block' 
        ? `Hi ${application.fullName}, your Campus Ambassador account has been blocked. Reason: ${application.adminComment}. Contact support.`
        : `Hi ${application.fullName}, your Campus Ambassador account has been unblocked. You can resume referring!`;

      // Push notification
      const notification = new Notification({
        title: action === 'block' ? "Ambassador Account Blocked" : "Ambassador Account Unblocked",
        body: smsMessage,
        priority: "High",
        targetGroup: "Specific",
        targetUsers: [user._id.toString()],
